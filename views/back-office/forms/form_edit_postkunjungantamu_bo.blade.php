...
<div class="section-header">
  <div class="section-header-back">
    <a href="{{route('post-kunjungan-tamu')}}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
  </div>
  <h1>Kembali Ke Halaman Pengolahan Konten Kunjungan Tamu</h1> 
</div>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4>Form Edit Post Kunjungan Tamu</h4>
        </div>
          ...
          @endif          
        <div class="card-body">
          <form id="form-post-content" action="{{ route('post-kunjungan-tamu-update',  $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf          
            @method('PUT')          
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <select class="form-control selectric @error('kategori') is-invalid @enderror" id="kategori" name="kategori">
                  <option value="{{ old('kategori', 'sop') }}" @if (old('kategori') == 'sop' || $data->kategori == 'sop') selected="selected" @endif>SOP</option>
                  <option value="{{ old('kategori', 'tata cara') }}" @if (old('kategori') == 'tata cara' || $data->kategori == 'tata cara') selected="selected" @endif>Tata Cara</option>
                </select>
                @error('kategori')
                    <div class="alert alert-danger alert-block small">
                      <strong>{{ $message }}</strong>
